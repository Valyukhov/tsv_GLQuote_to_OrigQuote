...
exports.convertTsv9to7 = convertTsv9to7;
exports.tsvtojson_ = void 0;

var _tsvToGroupData = require("./tsvToGroupData");
...
function trimValue(value) {
  if (typeof value === 'string') {
    value = value.replaceAll(_tsvToGroupData.HARD_NL, '\n');
    value = (0, _tsvToGroupData.trimWhiteSpace)(value);
  }
  ...
  try {
    if (tsvObjects && Array.isArray(tsvObjects) && tsvObjects.length) {
      ...
      for (let i = 0, l = tsvObjects.length; i < l; i++) {
        ...
        OrigQuote = OrigQuote.replaceAll(_tsvToGroupData.NO_BREAK_SPACE, ' '); // Replace non-break spaces

        OrigQuote = OrigQuote.replaceAll(_tsvToGroupData.ZERO_WIDTH_SPACE, ''); // Delete zero-width spaces

        OrigQuote = OrigQuote.replaceAll(_constants.THREE_DOTS, _constants.ELLIPSIS);
        OrigQuote = OrigQuote.replaceAll(' ' + _constants.ELLIPSIS, _constants.ELLIPSIS).replaceAll(_constants.ELLIPSIS + ' ', _constants.ELLIPSIS); // ELLIPSIS Should only be BETWEEN words

        if (OrigQuote[0] === _constants.ELLIPSIS) {
          OrigQuote = OrigQuote.substring(1);
        }

        if (OrigQuote[OrigQuote.length - 1] === _constants.ELLIPSIS) {
          OrigQuote = OrigQuote.substring(0, OrigQuote.length - 1);
        }

        OrigQuote = OrigQuote.replaceAll(_constants.ELLIPSIS, ' & ');
        ...
        if (OccurrenceNote.includes('  ') && !OccurrenceNote.includes('  *')) {
          // used in markdown for indenting
        ...
        if (GLQuote === 'Connecting Statement:' || GLQuote === 'General Information:' || GLQuote === 'A Bible story from:') {
        ...
        tsvObjects[i] = newTsvObject; // output_line = f'{reference}\t{ID}\t{tags}\t{support_reference}\t{orig_quote}\t{occurrence}\t{occurrence_note}'
      }

      const output = [];
      const headers = ['Reference', 'ID', 'Tags', 'SupportReference', 'Quote', 'Occurrence', 'Note'];
