...
exports.tnJsonToGroupData = tnJsonToGroupData;
exports.tsvToObjects = tsvToObjects;
exports.parseReference = parseReference;
exports.toInt = toInt;
exports.convertReference = convertReference;
exports.generateGroupDataItem = exports.cleanOccurrenceNoteLinks = exports.fixBibleLink = exports.convertLinkToMarkdownLink = exports.cleanGroupId = exports.trimWhiteSpace = exports.isExtraWhiteSpace = exports.tsvToGroupData7Cols = exports.tsvToGroupData = exports.HTML_BREAK2 = exports.HTML_BREAK = exports.HARD_NL = exports.ZERO_WIDTH_NO_BREAK_SPACE = exports.NO_BREAK_SPACE = exports.ZERO_WIDTH_SPACE = void 0;

var _pathExtra = _interopRequireDefault(require("path-extra"));

var _fsExtra = _interopRequireDefault(require("fs-extra"));

var tsvparser = _interopRequireWildcard(require("uw-tsv-parser"));

var _bibleReferenceRange = require("bible-reference-range");

var _tNoteGroupIdCategorization = require("./tNoteGroupIdCategorization");

var _ManageResourceAPI = _interopRequireDefault(require("./helpers/ManageResourceAPI"));

var _wordOccurrenceHelpers = require("./helpers/wordOccurrenceHelpers");

var _constants = require("./utils/constants");

var _resourcesHelpers = require("./helpers/resourcesHelpers");

var _ellipsisHelpers = require("./helpers/ellipsisHelpers");

var _verseHelpers = require("./helpers/verseHelpers");

var _tsvConversion = require("./tsvConversion");
...
exports.ZERO_WIDTH_NO_BREAK_SPACE = ZERO_WIDTH_NO_BREAK_SPACE;
const HARD_NL = `\\n`;
exports.HARD_NL = HARD_NL;
const HTML_BREAK = `<br>`;
exports.HTML_BREAK = HTML_BREAK;
const HTML_BREAK2 = `<br/>`;
/**
...
exports.HTML_BREAK2 = HTML_BREAK2;
...
function tnJsonToGroupData(originalBiblePath, bookId, tsvObjects, resourcesPath, langId, toolName, params, filepath) {
  const groupData = {};

  try {
    const resourceApi = new _ManageResourceAPI.default(originalBiblePath, bookId.toLowerCase());
    ...
    for (const tsvItem of tsvObjects) {
      if (tsvItem.SupportReference && tsvItem.OrigQuote) {
        ...
        try {
          ...
          if (parseInt(tsvItem.Chapter, 10) && parseInt(tsvItem.Verse, 10)) {
            // only if chapter and verse are valid do we expect verse text
    ...
    return params && params.categorized ? (0, _tNoteGroupIdCategorization.categorizeGroupData)(groupData) : groupData;
  } catch (e) {
...
const tsvToGroupData =
/*#__PURE__*/
function () {
  var _ref = _asyncToGenerator(function* (filepath, toolName, params = {}, originalBiblePath, resourcesPath, langId) {
    let tsv = _fsExtra.default.readFileSync(filepath, 'utf8');

    if (tsv.indexOf(HARD_NL) >= 0) {
      // see if we need to clean up file before calling library
      const cleanedTsv = tsv.replaceAll(HARD_NL, '\n');
      tsv = cleanedTsv;
    }

    const tsvObjects = (0, _tsvConversion.tsvtojson_)(tsv);
    const {
      Book: bookId
    } = tsvObjects[0] || {};
  ...
  return function tsvToGroupData(_x, _x2) {
...
exports.tsvToGroupData = tsvToGroupData;

function tsvToObjects(_x3) {
  return _tsvToObjects.apply(this, arguments);
}
/**
...
function _tsvToObjects() {
  _tsvToObjects = _asyncToGenerator(function* (tsvPath) {
    const tsvLines = _fsExtra.default.readFileSync(tsvPath).toString(); // console.log(tsvLines);
...
function parseReference(ref) {
  const cleanedRef = (0, _bibleReferenceRange.cleanupReference)(ref);
  const ref_ = {
...
const tsvToGroupData7Cols =
/*#__PURE__*/
function () {
  var _ref2 = _asyncToGenerator(function* (filepath, bookId, resourcesPath, langId, toolName, originalBiblePath, params) {
    const {
      tsvItems,
      error
    } = yield tsvToObjects(filepath, {});
  ...
  return function tsvToGroupData7Cols(_x4, _x5, _x6, _x7, _x8, _x9, _x10) {
...
exports.tsvToGroupData7Cols = tsvToGroupData7Cols;
...
exports.isExtraWhiteSpace = isExtraWhiteSpace;
...
const cleanGroupId = groupId => {
  try {
    ...
    let cleanedId = elements[elements.length - 1]; // Replace _ with - in groupId
...
exports.cleanGroupId = cleanGroupId;

const convertLinkToMarkdownLink = (tHelpsLink, resourcesPath, langId) => {
  try {
    ...
    let resourcePath = _pathExtra.default.join(resourcesPath, langId, 'translationHelps', _resourcesHelpers.translationHelps[resource]);

    resourcePath = (0, _resourcesHelpers.getLatestVersionInPath)(resourcePath);
    let articlePath;

    if (resource === 'ta') {
      articlePath = _pathExtra.default.join(resourcePath, category, file) + '.md';
    }

    if (resource === 'tw') {
      articlePath = _pathExtra.default.join(resourcePath, category.split('/')[1], 'articles', file) + '.md';
    }

    if (articlePath && _fsExtra.default.existsSync(articlePath)) {
      const groupName = (0, _resourcesHelpers.getGroupName)(articlePath);
...
exports.convertLinkToMarkdownLink = convertLinkToMarkdownLink;

const fixBibleLink = (link, resourcesPath, langId, bookId, chapter) => {
  const bibleId = (0, _resourcesHelpers.getBibleIdForLanguage)(_pathExtra.default.join(resourcesPath, langId, 'bibles'));
...
exports.fixBibleLink = fixBibleLink;

const cleanOccurrenceNoteLinks = (occurrenceNote, resourcesPath, langId, bookId, chapter) => {
  try {
    ...
    let cleanNote = occurrenceNote.replace(colonInPathPattern, '/'); // Remove spaces between the link and right paren
    ...
    cleanNote = cleanNote.replace(spaceBetweenLinkAndParenPattern, ')'); // Remove invalid paren and spaces at end of the link
    // Ex: [[rc://en/man/ta:translate:figs-activepassive )]] =>
    //     [[rc://en/man/ta/translate/figs-activepassive]]

    const invalidParenInLinkPattern = /(?<=\[\[rc:[^\] )]+)[ \)]+(?=]])/g;
    cleanNote = cleanNote.replace(invalidParenInLinkPattern, ')'); // Removes a right paren if it appears after one link and then there is another link
    ...
    cleanNote = cleanNote.replace(rightParenBetweenLinksPattern, ''); // Run cleanGroupId on the last item of the path, the groupId
    // Ex: [[rc://en/man/ta/translate/figs_activepassive]] =>
    //     [[rc://en/man/ta/translate/figs-activepassive]]

    const groupIdPattern = /(?<=\[\[rc:\/\/[^\]]+\/)[\w-]+(?=\]\])/g;
    cleanNote = cleanNote.replace(groupIdPattern, cleanGroupId); // Run convertLinkToMarkdownLink on each link to get their (title)[rc://...] representation
...
exports.cleanOccurrenceNoteLinks = cleanOccurrenceNoteLinks;
...
function convertReference(item) {
  ...
  const isSet = (0, _verseHelpers.isVerseSet)(itemVerse); // if list or range
...
const generateGroupDataItem = (tsvItem, toolName, verseString) => {
  ...
  const wordOccurrencesForQuote = (0, _wordOccurrenceHelpers.getWordOccurrencesForQuote)(OrigQuote, verseString, true); // uses tokenizer to get list of words handle various punctuation and spacing chars

  const quote = wordOccurrencesForQuote.length > 1 || (0, _ellipsisHelpers.hasEllipsis)(OrigQuote) ? wordOccurrencesForQuote : OrigQuote; // only use array if more than one word found

  const quoteString = OrigQuote.trim().replace(/\.../gi, _constants.ELLIPSIS);
  const {
    chapter,
    verse
  } = convertReference(tsvItem);
  return {
    comments: false,
    reminders: false,
    selections: false,
...
exports.generateGroupDataItem = generateGroupDataItem;