...
function verseObjectsToString(verseObjects) {
  return (// eslint-disable-next-line prettier/prettier
    ...
    .join('') // remove double spaces
    .replace(/ {2}/gi, ' ') // remove spaces before commas
    .replace(/ , /gi, ', ') // remove spaces before periods
    .replace(/ ."/gi, '."') // remove space before apostrophes
    .replace(/ ’./gi, '’.') // replace space before semicolon
    .replace(/ ; /gi, '; ') // remove spaces before question marks
    .replace(/\s+([?])/gi, '$1') // remove whitespace from the beginning
    .trimLeft()
  );
}