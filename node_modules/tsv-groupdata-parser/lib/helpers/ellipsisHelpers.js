...
exports.getOmittedWordsInQuote = getOmittedWordsInQuote;
exports.hasEllipsis = hasEllipsis;

var _constants = require("../utils/constants");

var _wordOccurrenceHelpers = require("./wordOccurrenceHelpers");
...
function regex(quote) {
  ...
  return new RegExp((0, _wordOccurrenceHelpers.cleanRegex)(quote), 'g');
}
...
function getQuoteChunkSubStrIndex(verseString, previousQuoteChunk, quoteChunk, nextQuoteChunk) {
  ...
  if (splittedVerse.length === 2 && matches.length === 1) {
    useLastIndexOf = splittedVerse[0].includes(quoteChunk); // eslint-disable-next-line prettier/prettier

    result = useLastIndexOf ? verseString.lastIndexOf(quoteChunk) : verseString.indexOf(quoteChunk);
...
function getOmittedWordsInQuote(quote, verseString) {
  // Clean quote string
  quote = (0, _stringHelpers.cleanQuoteString)(quote);
  const quoteChunks = quote.split(_constants.ELLIPSIS);
  ...
  quoteChunks.forEach((quoteChunk, index) => {
    ...
    let nextQuoteChunk = validateChunk(verseString, quoteChunks[index + 1]); // if first chunk & is not the last item in the array
    ...
    if (index === 0 && index < quoteChunks.length - 1) {
      ...
      const strBeforeNextQuote = getStrPrecedingPreviousChunk(verseString, previousQuoteChunk, quoteChunk, nextQuoteChunk) || ''; // TRICKY: in some cases the chunk isn't found in the preceding string because of extra space in the string.
      ...
      const useLastIndexOf = quoteChunks.length === index + 1; // eslint-disable-next-line prettier/prettier

      const lastMissingWordEndingIndex = useLastIndexOf ? verseString.lastIndexOf(quoteChunk) : verseString.indexOf(quoteChunk);
      ...
      let endIndex = useLastIndexOf ? verseString.lastIndexOf(quoteChunk) : verseString.indexOf(quoteChunk);
  ...
  wholeQuote = (0, _stringHelpers.cleanQuoteString)(wholeQuote);
  return {
    wholeQuote,
    omittedStrings
  };
...
function hasEllipsis(origQuote = '') {
  return origQuote.includes(_constants.ELLIPSIS);
}